...
@section('content')
    ...
    <nav class="navbar navbar-expand-md">
        <div class="container">
            <a class="navbar-brand" href="#">
                @if (isset($about->logo))
                    <img src="{{ asset('Admin/About/' . $about->logo) }}" alt="" width="120" height="100">
                @endif
            </a>
            ...
            <div class="cards-container">
                @foreach ($projects as $project)
                    ...
                    <div class="card-image">
                        <img src="{{ asset('Admin/Projects/' . $project->image) }}" class="card-img-top"
                            alt="{{ $project->image }}">
                    </div>
                    <div class="card-body">
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-6 left-footer">
                    <p>Copyright &copy; @if (isset($about->name))
                            {{ $about->name }}
                        @endif
                    </p>
                </div>
                <div class="col-sm-6">
                    <div class="right-footer">
                        @if (isset($social->facebook))
                            <a href="{{ $social->facebook }}" target="_blank" class="text-decoration-none"><i
                                    class="fa-brands fa-fw fa-facebook-f"></i></a>
                        @endif
                        @if (isset($social->whats))
                            <a href="https://api.whatsapp.com/send?phone={{ $social->whats }}" target="_blank"
                                class="text-decoration-none"><i class="fa-brands fa-fw fa-whatsapp"></i></a>
                        @endif
                        @if (isset($social->gmail))
                            <a href="mailto:{{ $social->gmail }}" target="_blank" class="text-decoration-none"><i
                                    class="fa-brands fa-google-plus-g"></i></a>
                        @endif
                        @if (isset($social->linkedin))
                            <a href="{{ $social->linkedin }}" target="_blank" class="text-decoration-none"><i
                                    class="fa-brands fa-fw fa-linkedin-in"></i></a>
                        @endif
                        @if (isset($social->twitter))
                            <a href="{{ $social->twitter }}" target="_blank" class="text-decoration-none"><i
    ...
    <script>
        ...
        projectModal.addEventListener("show.bs.modal", a => {
            const b = a.relatedTarget,
                c = b.querySelector("p").textContent,
                d = b.querySelector("span").textContent,
                e = b.dataset.id;
            document.getElementById("modal_title").textContent = c, document.getElementById("project_disc")
                .textContent = d, $.ajax({
            ...
            $.ajax({
                ...
                success: function(a) {
                    if ("true" === a.status) {
                        for (let b = 0; b < a.details.length; b++) {
                          if (a.details[b].image.split('.')[1] != 'mp4') {
                            c += `<a href="{{ asset('Admin/Details/${a.details[b].image}') }}">
                              <img src="{{ asset('Admin/Details/${a.details[b].image}') }}">
                            </a>`;
                          } else {
                            c += `<a href="{{ asset('Admin/Details/${a.details[b].image}') }}">
                              <video src="{{ asset('Admin/Details/${a.details[b].image}') }}" controls controlsList="nodownload"></video>
                            </a>`;
                          }
                        }
                        $("#lightgallery").html(c), lightGallery(document.getElementById("lightgallery"))
                    }
                }
            })
        ...
        let clinetOwl = $(".clients-carousel");
        clinetOwl.owlCarousel({
            loop: !0,
            margin: 10,
            autoplay: !0,
            autoplayTimeout: 2e3,
