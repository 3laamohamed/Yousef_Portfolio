...
<script>
  ...
  $('#update_project').on('click', function(e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if(data.status === "true") {
          $(`tr#${rowId}`).find('td:nth-child(2)').text($('#project_name').val());
          $('#save_project').removeClass('d-none');
    ...
    $.ajax({
      ...
      success: function(data) {
        if(data.status === "true") {
          ...
          $('#item-image').html(`<label for="image" class="image unvisibile" data-label="Add thumbnail" style="background-image:url({{ URL::asset('Admin/projects') }}/${data.msg.image})"></label>`);
          window.scrollTo({
            top: 0,
            behavior: 'smooth'
          });
